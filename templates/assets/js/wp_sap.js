...
var survey = [];
var survey_options = [];
if (sss_params.survey_options!=undefined) survey = JSON.parse(sss_params.survey_options);
if (survey.options!=undefined) survey_options = JSON.parse(survey.options);
var	played_question=-1;
var rmdni = false;
var lastScrollTop = 0;
var played = 0;
if (survey.align==undefined) survey.align='left';
if (survey.width==undefined) survey.width='98%';
if (survey.style!='flat') jQuery("body").prepend('<div id="survey" style="position: fixed;'+survey.align+':0px;z-index: 999999;width:'+survey.width+';padding:1%;bottom:-300px;"></div>');
	...
	{
		if (jQuery("#bglock").length==0) jQuery("body").append("<div id='bglock'></div>");
		if ((survey.expired!='true')&&(survey.style!='click')&&(survey_options[15]!=1)) jQuery("#bglock").fadeIn(1000);
	}
if (survey_options[15]!=1&&survey.style=='modal') setTimeout(function(){played_question=0;play_survey();},2000);
if (survey.style=='flat') {jQuery('#survey').hide();/*played_question=0;setTimeout(function(){play_survey();},2000);*/}
  ...
  (function() {        
	...
	{
	if(jQuery(window).scrollTop() + jQuery(window).height() > jQuery(document).height() - ((jQuery(document).height()/100)*10)&&jQuery(this).scrollTop() > lastScrollTop&&played_question==-1) {if (survey.style=='modal'&&survey_options[15]==1) 
	{
	if (parseInt(survey_options[13])==1) jQuery("#bglock").fadeIn(1000);
	played_question=0;play_survey();}
	}
	...
	{	
		if (survey_options[0]=="bottom") jQuery('#survey').animate({bottom: '10px'}, parseInt(survey_options[11]), 'easeOutBounce',function(){})
		if (survey_options[0]=="center") jQuery('#survey').animate({top: ((jQuery(window).height()-jQuery('#survey').height())/2)+"px"}, parseInt(survey_options[11]), 'easeOutBounce',function(){})
		if (survey_options[0]=="top") jQuery('#survey').animate({top: '10px'}, parseInt(survey_options[11]), 'easeOutBounce',function(){})
	}
    };
})();
