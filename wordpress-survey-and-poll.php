	...
	class wp_sap
	{
		...
		public function __construct()
		{
			...
			else
			{
				...
				$wp_sap_load = true;
				if ((strpos($wp_sap_url,'wp-login'))!==false) {$wp_sap_load = false;}
				if ((strpos($wp_sap_url,'wp-admin'))!==false) {$wp_sap_load = false;}
		...
		**/
		...
		public function survey_shortcodes( $atts ) {
			...
			extract( shortcode_atts( array(
				...
				if (!isset($atts['style'])) $atts['style'] = 'modal';
				if (!isset($atts['align'])) $atts['align'] = '';
				if (!isset($atts['width'])) $atts['width'] = '98%';
				if (!isset($atts['message'])) $atts['message'] = 'You already filled out this survey!';
			...
			wp_register_script('wp_sap_script', plugins_url('/templates/assets/js/wp_sap.js' , __FILE__ ), array('jquery'),'1.0.0.1',true );
					$survey_viewed = array();$sv = '';$sv_condition = '';
					if ($atts['style'] = 'click')
					{
					if (isset($_COOKIE['wp_sap'])) $survey_viewed = unserialize(stripslashes($_COOKIE['wp_sap']));
					if (!empty($survey_viewed)) {if (in_array($args['id'],$survey_viewed)) $sv_condition = "expired";}
			$sql = "SELECT *,msq.id as question_id FROM ".$wpdb->prefix."wp_sap_surveys mss LEFT JOIN ".$wpdb->prefix."wp_sap_questions msq on mss.id = msq.survey_id WHERE (`expiry_time`>'".current_time( 'mysql' )."' OR `expiry_time`='0000-00-00 00:00:00') AND (`start_time`<'".current_time( 'mysql' )."' OR `start_time`='0000-00-00 00:00:00') AND mss.id='".$args['id']."' ORDER BY msq.id ASC";
					...
					{
					if (isset($_COOKIE['wp_sap'])) $survey_viewed = unserialize(stripslashes($_COOKIE['wp_sap']));
					if (!empty($survey_viewed)) {$sv = implode($survey_viewed);$sv_condition = "AND (mss.id NOT IN ('".$sv."'))";}
			...
			foreach($questions_sql as $key=>$qs)
			{
				if ($key==0)
				{
					...
					if ($sv_condition!="expired") $survey['expired'] = 'false';
					else  $survey['expired'] = 'true';
					if ($args['style']=='click') $survey['message'] = $atts['message'];
				}
			...
			if ($args['style']=='flat') return('<div id="survey" style="width:100%;"></div>');
			}
		}
